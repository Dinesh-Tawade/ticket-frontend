  ...
  // ==================== SINGLE TICKET STUB ====================
  const SingleTicket = React.forwardRef(({ booking, seat, showDate, showTime, theater, show, timing }, ref) => {
    ...
    const displayDate = timing?.showDate || showDate;
    const displayTime = timing?.startTime || showTime;
    ...
    const qrData = `${bookingId}|${rowName}|${seatNumber}|${seatLabel}|${displayDate}|${displayTime}`;
        ...
        <div className="st-left">
          ...
          <div className="st-row">
            ...
            <div>
              ...
              <div className="st-val">{displayTime}</div>
            </div>
          </div>
          <div className="st-row">
  ...
  // ==================== TICKET MODAL ====================
  const TicketModal = ({ booking, show, theater, timing, onClose, onRedirect }) => {
        ...
        <div className="tm-wrap">
          ...
          <div className="tm-carousel">
            ...
            <SingleTicket 
              ...
              booking={booking} 
              seat={seats[idx]} 
              showDate={timing?.showDate || show?.showDate}
              showTime={timing?.startTime || show?.startTime}
              theater={theater}
              show={show}
              timing={timing}
            />
  ...
  // ==================== CINEMA SEAT FLOOR COMPONENT WITH BOOKED SEATS LOCK ====================
                    ...
                    return (
                      <span key={si} style={{ display: "contents" }}>
                        ...
                        <button
                          ...
                          onClick={() => !isAisle && !isBlocked && !isBooked && isAccessible && onToggle(fullKey, zone, seatNumber)}
                        ...
                        >
                          ...
                          {hoveredSeat === seatNumber && isSeatBooked && (
  ...
  const CinemaBookingPreview = ({ theater, show, accessibleSeats = [], onClose, onBookingSuccess }) => {
    ...
    const [showTimings, setShowTimings] = useState(show?.timings || []);
    const [bookedSeats, setBookedSeats] = useState(new Set());
    const [isLoadingSeats, setIsLoadingSeats] = useState(false);

    const accessibleSeatSet = useMemo(() => new Set(accessibleSeats || []), [accessibleSeats]);

    // Fetch booked seats for selected timing
    const fetchBookedSeats = useCallback(async (timingId) => {
    ...
    // When timing changes, fetch booked seats
    useEffect(() => {
      if (selectedTiming?._id) {
        fetchBookedSeats(selectedTiming._id);
        setSelected(new Set()); // Clear selected seats when timing changes
      }
    }, [selectedTiming, fetchBookedSeats]);
    ...
    const createBookingMutation = useMutation({
      ...
      onSuccess: (response) => {
        ...
        // Update booked seats set with newly booked seats
        const newBookedSeats = new Set(bookedSeats);
        selectionInfo.labels.forEach(seat => newBookedSeats.add(seat));
        setBookedSeats(newBookedSeats);
        setSelected(new Set()); // Clear selected seats
    ...
    const toggleSeat = useCallback((fullKey, zone, seatNumber) => {
    ...
    const handleBooking = useCallback(() => {
      ...
      const alreadyBooked = selectionInfo.labels.find((s) => bookedSeats.has(s));
      if (alreadyBooked) {
        toast.error(`Seat ${alreadyBooked} is already booked!`);
        return;
      }

      createBookingMutation.mutate({
        showId: show._id,
        timingId: selectedTiming._id,
        seats: selectionInfo.seatsPayload,
      });
    }, [accessibleSeatSet, createBookingMutation, selectionInfo, show, selectedTiming, bookedSeats]);
      ...
      >
        ...
        >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: "#9ca3af" }}>
                  {theater?.name} · {show?.screenNumber ? `Screen ${show.screenNumber}` : ''}
                </div>
              </div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            {[
              { color: "#22c55e28", border: "#22c55e", label: "Your Seats", icon: null },
              { color: "#4a9edd28", border: "#4a9edd70", label: "Available", icon: null },
              { color: "#dc2626", border: "#b91c1c", label: "Booked & Locked", icon: <FaLock size={10} color="#fff" /> },
              { color: "#2a2a38", border: "#3a3a48", label: "No Access", icon: null },
            ].map((l) => (
              <div key={l.label} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#9ca3af" }}>
                <div style={{ width: 16, height: 16, borderRadius: 4, background: l.color, border: `1.5px solid ${l.border}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {l.icon}
                </div>
                {l.label}
              </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", background: "#0f0f16", padding: "0 24px 20px" }}>
          {showTimings && showTimings.length > 0 && (
            <div style={{ marginBottom: 24 }}>
              <div style={{ 
                ...
                <div style={{ fontSize: 13, fontWeight: 600, color: "#60a5fa", marginBottom: 12, display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
                  {showTimings.map((timing, idx) => {
                    ...
                    return (
                      <button
                        key={timing._id || idx}
                        onClick={() => !isPast && setSelectedTiming(timing)}
                        disabled={isPast}
                        style={{
                          flex: "1 1 auto",
                          minWidth: "180px",
                          ...
                          border: `1.5px solid ${isSelected ? "#22c55e" : "rgba(255,255,255,0.1)"}`,
                      ...
                      >
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                          <FaCalendarAlt size={12} style={{ color: isSelected ? "#22c55e" : "#9ca3af" }} />
                          <span style={{ fontSize: 13, fontWeight: 600, color: isSelected ? "#22c55e" : "#fff" }}>
                            {formatDate(timing.showDate)}
                          </span>
                        </div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <FaClock size={12} style={{ color: isSelected ? "#22c55e" : "#9ca3af" }} />
                          <span style={{ fontSize: 14, fontWeight: 700, color: isSelected ? "#22c55e" : "#e5e7eb" }}>
          ...
          {isLoadingSeats ? (
            <div style={{ textAlign: "center", padding: 60 }}>
              ...
              <p style={{ color: "#9ca3af", marginTop: 16 }}>Loading seat availability...</p>
            </div>
          ) : !hasLayout ? (
        ...
        >
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button
              ...
              disabled={selectionInfo.count === 0 || createBookingMutation.isPending || !selectedTiming}
              style={{
                padding: "10px 28px",
                borderRadius: 8,
                border: "none",
                background: selectionInfo.count === 0 || !selectedTiming ? "#2a2a38" : "#e2c97e",
                color: selectionInfo.count === 0 || !selectedTiming ? "#6b7280" : "#0f0f16",
                cursor: selectionInfo.count === 0 || !selectedTiming ? "not-allowed" : "pointer",
                fontSize: 14,
                fontWeight: 700,
                  ...
                  {visibleTimings?.map((timing, idx) => (
                    ...
                    >
                      ...
                      <div className="text-xs font-bold text-emerald-500">
                        ₹{timing.seatCategories?.[0]?.pricePerSeat || show.basePrice || 150}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium" style={{ color: "var(--foreground)", opacity: 0.6 }}>
                      Select a show timing and book from the seats assigned to your account.
                    </p>
                  </div>
                </div>
