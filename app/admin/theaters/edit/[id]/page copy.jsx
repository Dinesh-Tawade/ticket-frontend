...
//   const [rowNaming,   setRowNaming]   = useState(initialData?.rowNaming || "alpha");
//   const [zones,       setZones]       = useState(initialData?.zones || DEFAULT_ZONES);
//   const [tool,        setTool]        = useState("paint");
//   const [activeZone,  setActiveZone]  = useState(initialData?.zones?.[0]?.id || "z1");
...
//   const [groundRows, setGroundRows] = useState(initialData?.levels?.ground?.rows || 13);
//   const [groundCols, setGroundCols] = useState(initialData?.levels?.ground?.cols || 14);
//   const [balconyRows, setBalconyRows] = useState(initialData?.levels?.balcony?.rows || 6);
//   const [balconyCols, setBalconyCols] = useState(initialData?.levels?.balcony?.cols || 14);

//   const [aisleCols, setAisleCols]         = useState(initialData?.aisleCols || []);
//   const [aisleRows, setAisleRows]         = useState(initialData?.aisleRows || []);
...
//         // Initialize layout data from existing theater
//         if (t.layoutMeta) {
//           setInitialLayoutData(t.layoutMeta);
//         }
//       }
//     }
//   });
...
function TheaterLayoutBuilder({ onLayoutChange, initialData }) {
  ...
  useEffect(() => {
    ...
    if (initialData.aisleCols?.length) setAisleCols(initialData.aisleCols);
    if (initialData.aisleRows?.length) setAisleRows(initialData.aisleRows);
...
export default function EditTheaterPage() {
  ...
  const preparePayload = () => {
    ...
    return {
      ...
      images: [],
      layoutMeta: ld ? {
        aisleCols:        ld.aisleCols,
        aisleRows:        ld.aisleRows,
